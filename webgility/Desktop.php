...
class Webgility_Shop1_Model_Runmobile extends WgCommon
{
	...
	function checkAccessInfo($username,$password,$others)
	{
		...
		$responseArray['StatusCode'] = $code;
			if($version!==0)
			{
				if((version_compare($version, '1.2.1',"<") > 0) || (version_compare($version, "1.13.1",">=") > 0))
				{
...
****************************************************************************************************/
				...
				/* 	$pp= Mage::getModel("catalog/product")->load($iInfo['product_id']);
						//print_r($pp);
 $new_items = Mage::getModel('giftwrap/selection')->getSelectionByQuoteId($orders['quote_id']);

$style = Mage::getModel('giftwrap/giftwrap')->load(2);
			$giftcard = Mage::getModel('giftwrap/giftcard')->load($new_items[1]['id']);  */
				...
				if($orders["giftwrap_amount"])
				{
		
					$itemI++;					
					$Item = new Item();
					$Item->setItemCode($RewardsPoints_Name);
					$Item->setItemDescription("giftwrap amount");
					$Item->setItemShortDescr("giftwrap amount");
					$Item->setQuantity(intval(1));
					$Item->setUnitPrice($orders["giftwrap_amount"]);
					$Item->setWeight('');
					$Item->setFreeShipping("N");					
					$Item->setshippingFreight("0.00");
					$Item->setWeight_Symbol("lbs");
...
//	print_r($this->response($Orders->getOrders()));
									...
									if(isset($invoice[0]) && $invoice[0]!='')	
									 {
									 	...
									 	$this->CreateCreditMemo($order['OrderID']);
	...
	public  function CreateCreditMemo($orderId,$storeId=1)
	{
		...
		foreach ( $items as $item ) {
			...
			$item_array[$item->getId()] = $item->getQtyInvoiced ();
		...
		$order ->addStatusToHistory ( $orderCreditMemoStatusCode, $orderCreditMemoStatusComment, false );
		$order->setStatus($orderCreditMemoStatusCode);
		
		$order ->save ();
		if($order->canCancel())
		{ 
			$order->cancel()->save();	
		//$order->setState(Mage_Sales_Model_Order::STATE_CANCELED, true, 'Cancelled order due to refund');
		}
