...
class MageWorx_AbCart_Model_Cart extends Mage_Sales_Model_Quote
{
     ...
     * @return object
     */
    public function getNewCardCollection()
    {
        $collection = $this->getCollection();
        $time =  Mage::helper('mageworx_abcart')->getForeverAbandonedTime();
        $collection
            ->addFieldToFilter(
                'updated_at',
                array(
            ...
            ->getSelect()
            ->where('abcart_status IS NULL OR abcart_status IN(?)', array(self::CART_STATUS_ACTIVE));
