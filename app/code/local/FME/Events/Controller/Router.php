    ...
    public function match(Zend_Controller_Request_Http $request) 
    {
        ...
        if ($currRouter == $identifierNew) {
            ...
            if ($isPfx > 0) {
                ...
                if (isset($explode[2]) && strtotime($explode[2])) {
                    $date = str_replace(',', ' ', $explode[2]);
                    $request->setParam('repeat', $date);
                }
                
                $request->setAlias(
                    Mage_Core_Model_Url_Rewrite::REWRITE_REQUEST_PATH_ALIAS, $currRouter
                );
