...
class AW_Popup_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        $userAgentList = Mage::getStoreConfig('popup/settings/ignore_user_agent');
        $userAgentList = str_replace(array("\n", "\r"), ' ', $userAgentList);
        if (!$userAgentArray = explode(' ', $userAgentList)) {
            return null;
        }
        $result = array();
        foreach ($userAgentArray as $item) {
            if (!empty($item)) {
                $result[] = mb_strtolower(trim($item));
            }
        }
        return $result;
    }

    public function hideByUserAgent()
    {
        $userAgentArray = $this->getIgnoreUserAgent();
        if ($userAgentArray && is_array($userAgentArray)) {
            $clientUserAgent = mb_strtolower(Mage::helper('core/http')->getHttpUserAgent());
            foreach ($userAgentArray as $item) {
                if (strpos($clientUserAgent, $item) !== false) {
                    return true;
                }
            }
        }
    ...
    public function isNeedToShowOnMobileDevices()
    {
        return Mage::getStoreConfig('popup/settings/show_on_mobile_devices');
    }

    public function hideByMobileDevice()
    {
        $needToHide = false;
        if (!($this->isNeedToShowOnMobileDevices())) {
            $needToHide = $this->_isMobile();
        }
        return $needToHide;
    }
    ...
    {
        $collection = Mage::getModel('popup/popup')->getPopupCollection();
        ...
        foreach ($collection as $_item) {
            $showPopup = false;
            $compareUrl = $_item->getData('url_contains');
            if ($_item->getData('display_at') == AW_Popup_Model_Source_Display::URL_ID
                && !empty($compareUrl)
                && strpos($refererUrl, $compareUrl) !== false
            ) {
                $showPopup = true;

            }
            if ($_item->getData('display_at') != AW_Popup_Model_Source_Display::URL_ID
                && ($pageName = Mage::app()->getRequest()->getParam('page'))
                && in_array(Mage::getModel('popup/source_page')->getPageIDByName(Mage::helper('core')->escapeHtml($pageName)), $_item->getData('show_at'))
            ) {
                $showPopup = true;
