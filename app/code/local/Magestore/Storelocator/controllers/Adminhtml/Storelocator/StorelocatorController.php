...
class Magestore_Storelocator_Adminhtml_storelocator_StorelocatorController extends Mage_Adminhtml_Controller_Action {
    ...
    public function indexAction() {
		// $storelocators = Mage::getModel('storelocator/storelocator')->getCollection();
		// $i=0;
		// $array = array();
		// foreach($storelocators as $_store){
			// $i++;
			// $key = $this->Slug($_store->getData('name'));
			// if(in_array($key,$array)){
				// $key = $key.'-'.$i;
			// }else{
				// $array[] = $key;
			// }
			// $_store->setData('rewrite_request_path')->save()->updateUrlKey($key);
		// }
		// die('asdf');
	...
	public function massReNewUrlAction() {
        $storelocatorIds = $this->getRequest()->getParam('storelocator');
        if (!is_array($storelocatorIds)) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('Please select item(s)'));
        } else {
            try {
                foreach ($storelocatorIds as $storelocatorId) {
                    $storelocator = Mage::getSingleton('storelocator/storelocator')
                        ->load($storelocatorId);
                    $storelocator->setData('rewrite_request_path',$storelocator->getName())
                        ->updateUrlKey()
                        ->save();
                }
                $this->_getSession()->addSuccess(
